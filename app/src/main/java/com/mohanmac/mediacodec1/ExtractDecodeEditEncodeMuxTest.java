...
public class ExtractDecodeEditEncodeMuxTest {
    private static ExtractDecodeEditEncodeMuxTest extractDecodeEditEncodeMuxTest = null;
    private ExtractDecodeEditEncodeMuxTest(){};
    public static ExtractDecodeEditEncodeMuxTest getInstance(){
        if(extractDecodeEditEncodeMuxTest == null){
            synchronized (ExtractDecodeEditEncodeMuxTest.class){
                if(extractDecodeEditEncodeMuxTest == null){
                    extractDecodeEditEncodeMuxTest = new ExtractDecodeEditEncodeMuxTest();
                }
                return extractDecodeEditEncodeMuxTest;
            }
        }else{
            return extractDecodeEditEncodeMuxTest;
        }
    ...
    public void testExtractDecodeEditEncodeMux720p(String inputFile) throws Throwable {
        setSize(1280, 720);
        setSource(inputFile);
        setCopyVideo();
        setCopyAudio();
        ...
        try {
            if (mCopyVideo) {
                ...
                if(videoInputTrack == -1) {
                    Log.d(TAG,"Video Track not available");
                    return;
                }
...
//                assertTrue("missing audio track in test video", audioInputTrack != -1);
                if(audioInputTrack == -1){
                    Log.d(TAG,"Audio Track not available");
                    return;
                }
    ...
    private void doExtractDecodeEditEncodeMux(
        ...
        while ((mCopyVideo && !videoEncoderDone) || (mCopyAudio && !audioEncoderDone)) {
            ...
            while (mCopyVideo && !videoEncoderDone
                ...
                if(!muxing){
                    Log.d(TAG,"should have added track before processing output");
                    return;
                }
                if (VERBOSE) {
            ...
            while (mCopyAudio && !audioEncoderDone
                ...
                if(!muxing){
                    Log.d(TAG,"should have added track before processing output [927]");
                    return;
                }
                if (VERBOSE) {
        ...
        if (mCopyVideo) {
            if(videoDecodedFrameCount != videoEncodedFrameCount){
                Log.d(TAG,"encoded and decoded video frame counts should match [982]");
                return;
            }
